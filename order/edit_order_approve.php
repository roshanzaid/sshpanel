<?php
    ...
    isset($_POST['_salesAgreement']))
    {
        ...
        if(!empty($_eid))
        {
            ...
            if($result){
                if(!empty($_saName)){
                    $agreementQuery = $conn->query("UPDATE sales_agreement SET del_date = '".$_editDeliveryDate."', sales_agreement_path = '$_saName' WHERE order_id = '$_eid'");
                    $response['status'] = 1;
                }else{
                    $agreementQuery = $conn->query("UPDATE sales_agreement SET del_date = '".$_editDeliveryDate."', sales_agreement_path = 'Not Exists' WHERE order_id = '$_eid");
                    $response['status'] = 1;
                }
